...
export default function BackgroundDemoPage() {
  ...
  const handleApplyToDashboard = () => {
    ...
    // Setze die CSS-Variablen für das aktuelle Design (für Hintergründe)
    document.documentElement.style.setProperty("--color-primary", selectedColors?.primary || "#4facfe");
    document.documentElement.style.setProperty("--color-secondary", selectedColors?.secondary || "#00f2fe");
    document.documentElement.style.setProperty("--color-accent", selectedColors?.accent || "#a855f7");
    
    // Setze die Tailwind CSS-Variablen (--primary, --secondary, --accent) für die Website-Farben
    if (selectedColors?.primary) {
      const primaryHsl = hexToHsl(selectedColors.primary);
      document.documentElement.style.setProperty("--primary", primaryHsl);
      // Berechne eine passende foreground-Farbe (hell oder dunkel je nach Luminanz)
    ...
    if (selectedColors?.secondary) {
      const secondaryHsl = hexToHsl(selectedColors.secondary);
    ...
    if (selectedColors?.accent) {
      const accentHsl = hexToHsl(selectedColors.accent);
    ...
    // Füge die Hintergrund-Klasse zum body oder root-Element hinzu
    const rootElement = document.getElementById("root");
    if (rootElement) {
      // Entferne alle alten Hintergrund-Klassen
      backgroundOptions.forEach(opt => {
        rootElement.classList.remove(opt.className);
      });
      // Füge die neue Klasse hinzu
      if (selectedOption) {
        rootElement.classList.add(selectedOption.className);
      }
    }
