...
import { TrendingUp, TrendingDown, Activity, Users, Zap, Clock, ArrowUpRight, ArrowDownRight, Plus, Edit2, Trash2, X, Download, Save, Upload, Maximize2, Star, StarOff, FileSpreadsheet, FileText, LayoutTemplate, GitCompare, Sparkles } from "lucide-react";
...
import { exportDashboardAsPDF, exportDashboardToExcel, type DashboardData as ExportDashboardData } from "@/lib/dashboardExport";
...
function AnimatedValue({ value, isHighlight }: { value: string; isHighlight: boolean }) {
  ...
  return (
    <p className={cn("text-2xl font-bold transition-all duration-300", isHighlight && "text-white")}>
      {displayValue}
    </p>
  );
}
...
function SortableStatCard({ 
  ...
  return (
    <Card 
      ref={setNodeRef} 
      style={cardStyle}
      className={isHighlight ? "border-2 border-transparent" : ""}
    >
      <CardContent className="p-6 relative">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            {stat.trend === "up" ? (
              <div className={cn("flex items-center gap-1 text-sm", isHighlight ? "text-white" : "text-green-600")}>
          ...
          <div className="space-y-1">
            ...
            <p className={cn("text-sm", isHighlight ? "text-white/90" : "text-muted-foreground")}>{stat.title}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
function SortableActivityItem({ activity, isEditing, onEdit, onDelete }: { activity: Activity; isEditing: boolean; onEdit: () => void; onDelete: () => void }) {
...
function SortableWorkflowItem({ workflow, isEditing, onEdit, onDelete }: { workflow: TopWorkflow; isEditing: boolean; onEdit: () => void; onDelete: () => void }) {
...
export default function DashboardPage() {
    ...
    if (user) {
      try {
        ...
        if (saved) {
          ...
          if (saved.chartData) setChartData(saved.chartData);
          if (saved.activities) setActivities(saved.activities);
          if (saved.topWorkflows) setTopWorkflows(saved.topWorkflows);
  ...
  const handleImportDashboard = () => {
    ...
    input.onchange = (e) => {
      ...
      reader.readAsText(file);  <div className="flex flex-col items-center gap-2 text-muted-foreground">
  ...
  return (
    <div ref={dashboardRef} className="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-6 min-h-full w-full overflow-x-auto" style={{ backgroundColor: 'hsl(var(--background))' }}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between flex-wrap gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold" style={{ color: 'hsl(var(--foreground))' }}>Dashboard</h1>
          <p className="text-xs sm:text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>Verwalten Sie Ihre eigenen Daten</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
            ...
            <>
              ...
              >
                ...
                <span className="hidden sm:inline">Layout speichern</span>
              ...
              >
                ...
                <span className="hidden sm:inline">Layout laden</span>
          ...
          >
            ...
            <span className="hidden sm:inline">{isEditMode ? "Bearbeitung beenden" : "Bearbeiten"}</span>
            <span className="sm:hidden">{isEditMode ? "Fertig" : "Edit"}</span>
          </Button>
        </div>
      </div>
            ...
            return (
              <Card key={stat.id} className="relative group">
                <CardContent className="p-4 sm:p-6">
                  ...
                  <div className="space-y-1">
                    <AnimatedValue value={stat.value} isHighlight={false} />
        ...
        <DialogContent>
          ...
          {editingStat && (
            <div className="space-y-4">
              <div>
                <Label>Titel</Label>
                <Input value={editingStat.title} onChange={(e) => setEditingStat({ ...editingStat, title: e.target.value })} />
              </div>
              <div>
                <Label>Wert</Label>
                <Input value={editingStat.value} onChange={(e) => setEditingStat({ ...editingStat, value: e.target.value })} />
              </div>
              <div>
                <Label>Änderung</Label>
                <Input value={editingStat.change} onChange={(e) => setEditingStat({ ...editingStat, change: e.target.value })} />
              </div>
              <div>
                <Label>Trend</Label>
                <select
                  value={editingStat.trend}
                  onChange={(e) => setEditingStat({ ...editingStat, trend: e.target.value as "up" | "down" })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingStat({ ...editingStat, icon: e.target.value as StatCard["icon"] })}
                  className="w-full rounded-md border border-input bg-background px-3 py-2"
                >
            ...
            {editingChart && editingChart.name === "" && (
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={editingChart.name} onChange={(e) => setEditingChart({ ...editingChart, name: e.target.value })} />
                </div>
                <div>
                  <Label>Workflows</Label>
                  <Input type="number" value={editingChart.workflows} onChange={(e) => setEditingChart({ ...editingChart, workflows: parseInt(e.target.value) || 0 })} />
                </div>
                <div>
                  <Label>Ausführungen</Label>
                  <Input type="number" value={editingChart.executions} onChange={(e) => setEditingChart({ ...editingChart, executions: parseInt(e.target.value) || 0 })} />
                </div>
              </div>
            )}
          </div>
          ...
          {editingActivity && (
            <div className="space-y-4">
              <div>
                ...
                <Input value={editingActivity.workflow} onChange={(e) => setEditingActivity({ ...editingActivity, workflow: e.target.value })} />
              </div>
              <div>
                <Label>Status</Label>
                <select
                  value={editingActivity.status}
                  onChange={(e) => setEditingActivity({ ...editingActivity, status: e.target.value as Activity["status"] })}
                  className="w-full rounded-md border border-input bg-background px-3 py-2"
                >
              ...
              <div>
                <Label>Zeit</Label>
                <Input value={editingActivity.time} onChange={(e) => setEditingActivity({ ...editingActivity, time: e.target.value })} placeholder="z.B. vor 5 Min" />
              </div>
            </div>
          )}
          <DialogFooter>
        ...
        <DialogContent>
          ...
          {editingWorkflow && (
            <div className="space-y-4">
              <div>
                <Label>Name</Label>
                <Input value={editingWorkflow.name} onChange={(e) => setEditingWorkflow({ ...editingWorkflow, name: e.target.value })} />
              </div>
              <div>
                <Label>Ausführungen</Label>
                <Input type="number" value={editingWorkflow.executions} onChange={(e) => setEditingWorkflow({ ...editingWorkflow, executions: parseInt(e.target.value) || 0 })} />
              </div>
              <div>
                <Label>Erfolgsrate (%)</Label>
                <Input type="number" min="0" max="100" value={editingWorkflow.success} onChange={(e) => setEditingWorkflow({ ...editingWorkflow, success: parseInt(e.target.value) || 0 })} />
              </div>
            </div>
          )}
          <DialogFooter>
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-6">
                    <div className={cn(
                      ...
                      {React.createElement(iconMap[fullscreenWidget.data.icon], { className: "h-8 w-8 text-primary" })}
                    </div>
