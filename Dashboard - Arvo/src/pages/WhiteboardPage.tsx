...
export default function WhiteboardPage() {
  ...
  useEffect(() => {
    ...
    // Initialisiere History nach dem Resize
    setTimeout(() => {
      const ctx = canvas.getContext("2d");
    ...
    // Lade gespeicherte Farb-Nutzung
  ...
  const draw = useCallback((e: React.MouseEvent<HTMLCanvasElement> | MouseEvent) => {
    ...
    // Berücksichtige Zoom bei der Position
    const x = (e.clientX - rect.left) / zoom;
    const y = (e.clientY - rect.top) / zoom;

    if (tool === "eraser") {
      // Radiergummi wie bei Miro: runde Form, die Pixel löscht
      ctx.globalCompositeOperation = "destination-out";
      ctx.lineWidth = strokeWidth * 2; // Radiergummi ist etwas größer
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      
      // Zeichne eine Verbindungslinie zwischen den Punkten
      ctx.beginPath();
      ctx.moveTo(startPos.x, startPos.y);
      ctx.lineTo(x, y);
      ctx.stroke();
      
      // Zeichne zusätzlich einen Kreis am Endpunkt für bessere Abdeckung
      ctx.beginPath();
      ...
      if (isShiftPressed && startPos) {
        // Gerade Linie bei gedrückter Shift-Taste
        // Restore vom aktuellen History-Step (beim MouseDown gespeichert)
        ...
        // Jeder neue Mausklick startet einen neuen Strich
        ctx.beginPath();
        ctx.moveTo(startPos.x, startPos.y);
        ctx.lineTo(x, y);
        ctx.stroke();
        // Aktualisiere startPos nur für die nächste Linien-Segment während dieses Strichs
        setStartPos({ x, y });
      }
    }
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "text") {
      // Öffne Text-Eingabefeld an der Klickposition
      const canvas = canvasRef.current;
      if (!canvas) return;
      ...
      // Berechne Position relativ zum Container (mit Zoom berücksichtigt)
      ...
      // Focus nach kurzer Verzögerung, damit das Input-Element gerendert ist
      setTimeout(() => {
        textInputRef.current?.focus();
        textInputRef.current?.select();
      }, 100);
    } else if (tool === "pen") {
      // Sicherstellen, dass wir nicht mehr zeichnen (falls vorher noch aktiv)
      if (isDrawing) {
        setIsDrawing(false);
      }
      saveToHistory(); // Wichtig: Erst History speichern, dann zeichnen
      setStartPos({ x, y }); // Setze neuen Startpunkt
      setIsDrawing(true); // Beginne neuen Strich
    } else if (tool === "eraser") {
      // Sicherstellen, dass wir nicht mehr radieren (falls vorher noch aktiv)
      if (isDrawing) {
        setIsDrawing(false);
      }
      saveToHistory(); // Wichtig: Erst History speichern, dann radieren
      setStartPos({ x, y }); // Setze neuen Startpunkt
      setIsDrawing(true); // Beginne neues Radieren
  ...
  const handleMouseUp = () => {
    if (tool === "pen") {
      ...
      setIsDrawing(false);
      setStartPos(null);
      saveToHistory(); // Speichere nach dem Radieren, damit Undo funktioniert
    } else if (tool === "rectangle" || tool === "circle") {
      setIsDrawing(false);
      if (startPos) {
        saveToHistory();
  ...
  const handleTextSubmit = () => {
    ...
    // Wenn kein Text eingegeben wurde, einfach abbrechen
    if (!tempText.trim()) {
      setTextInput(null);
  ...
  const updateColorUsage = (newColor: string) => {
    setColorUsage((prev) => {
      ...
      // Sortiere nach Verwendung und behalte Top 10
  ...
  const handleToggleFullscreen = () => {
    if (!isFullscreen) {
      // Versuche verschiedene Fullscreen-APIs
  ...
  // Zoom-Wheel-Support (Ctrl/Cmd + Mouse Wheel)
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
