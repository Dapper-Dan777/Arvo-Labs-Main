...
interface Channel {
  ...
  unread?: number;
  pinned?: boolean;
  memberIds: string[]; // IDs der zugewiesenen Mitglieder
  description?: string;
...
export default function TeamPage() {
  ...
  const handleSendMessage = () => {
    ...
    const currentUser = teamMembers[0]; // Aktueller Benutzer (Admin)
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // In einer echten App würde hier die Datei hochgeladen werden
  ...
  // Beliebte Emojis für den Picker
  const popularEmojis = [
    "😀", "😂", "🥰", "😎", "🤔", "😊", "👍", "❤️", "🔥", "✨",
    "🎉", "✅", "❌", "⚠️", "💡", "🚀", "📝", "📎", "🔔", "⭐",
    "💯", "🎯", "👏", "🙌", "🤝", "💪", "🎊", "🎈", "🎁", "🍕"
  ...
  const handleCreateChannel = () => {
    const newChannel: Channel = {
      ...
      type: "public",
      unread: 0,
      memberIds: ["1"], // Admin wird automatisch hinzugefügt
    };
  ...
  const handleArchiveChannel = (channelId: string) => {
    ...
    if (channel) {
      // Optional: Channel aus der Liste entfernen oder als archiviert markieren
      setChannels(channels.filter((c) => c.id !== channelId));
      if (activeChannel === channelId) {
        // Wenn der aktive Channel archiviert wurde, zum ersten verfügbaren wechseln
  ...
  const handleChannelSelect = (channelId: string) => {
    // Setze den aktiven Channel
    setActiveChannel(channelId);
    // Sidebar schließen beim Klick auf einen Kanal
