import { Clock, Play, Pause, MoreHorizontal, Filter, Plus, Edit, Trash2, Calendar as CalendarIcon } from "lucide-react";
...
import { format, getISOWeek, getWeek, startOfWeek, eachDayOfInterval, eachWeekOfInterval, getISOWeekYear } from "date-fns";
...
export default function TriggersPage() {
  const sectionGradient = useSectionGradient(); // Triggers-Gradient: var(--gradient-triggers)
  ...
  // Generiere 30-Minuten-Schritte für Time-Picker
  ...
  // Track Nutzungszeit für Chart
  useEffect(() => {
    // Lade gespeicherte Nutzungszeit-Daten
    const savedData = localStorage.getItem("triggers-usage-data");
    if (savedData) {
      try {
        const parsed = JSON.parse(savedData);
        // Stelle sicher, dass alle Einträge ein timestamp haben (Rückwärtskompatibilität)
        ...
        // Wenn keine Daten vorhanden oder zu wenige, erstelle neue
      ...
      // Erstelle Beispiel-Daten für die letzten 365 Tage (für alle Zeiträume)
      const today = new Date();
      const exampleData = Array.from({ length: 365 }, (_, i) => {
        const date = new Date(today);
        date.setDate(date.getDate() - (364 - i));
        return {
          date: date.toLocaleDateString("de-DE", { weekday: "short", day: "2-digit" }),
          minutes: Math.floor(Math.random() * 120) + 30, // 30-150 Minuten
          timestamp: date.getTime(),
        };
      });
      setUsageChartData(exampleData);
    ...
    // Track aktuelle Nutzungszeit (alle 1 Minute)
    const startTime = Date.now();
    const interval = setInterval(() => {
      const minutesUsed = Math.floor((Date.now() - startTime) / 60000);
      // Update heutigen Eintrag
      const today = new Date();
      ...
      setUsageChartData(prev => {
        ...
        if (todayIndex >= 0) {
          ...
          updated.push({ date: todayStr, minutes: minutesUsed, timestamp: todayTimestamp });
          // Behalte nur die letzten 365 Tage
  ...
  // Gefilterte Trigger basierend auf Suche und Filtern
      ...
      const updated = prevTriggers.map(trigger => {
        if (trigger.id === id) {
          ...
          if (newStatus === "paused") {
            ...
            // Berechne nächsten Run basierend auf Typ
  ...
  const handleCreateTrigger = () => {
    ...
    // Formatiere Zeit-String basierend auf Typ
    let timeString = "";
  ...
  const handleEditTrigger = (id: number) => {
    const trigger = triggers.find(t => t.id === id);
    if (trigger) {
      // Parse Zeit-String für Schedule-Trigger
      let parsedDate: Date | undefined = undefined;
      let parsedTime = trigger.time;
      
      if (trigger.type === "Schedule" && trigger.time) {
        // Versuche Datum und Zeit zu parsen (z.B. "Monday 08:00" oder "09:00 AM")
        const dateMatch = trigger.time.match(/(\w+)\s+(\d{1,2}):(\d{2})/);
        if (dateMatch) {
          // Wenn Wochentag vorhanden, verwende nächsten entsprechenden Tag
          const today = new Date();
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    // Suche wird automatisch durch filteredTriggers angewendet
  };
  ...
  // Filtere und transformiere Chart-Daten basierend auf ausgewähltem Zeitraum
  const getFilteredChartData = () => {
    // Stelle sicher, dass wir gültige Daten haben
    const validData = usageChartData.filter(d => d && d.timestamp);
    
    if (!validData.length) {
      // Fallback: Leeres Array, Chart zeigt "Lade Chart-Daten..."
      return [];
    }
    ...
    switch (chartPeriod) {
      case "day":
        // Letzte 24 Stunden in Stunden-Gruppen
      ...
      case "week":
        // Letzte 7 Tage - zeige immer alle 7 Wochentage
        startDate = new Date(today);
      ...
      case "month":
        // Vorherige 2 Wochen, aktuelle Woche und nächste Woche = 4 Wochen
        const currentWeekStart = startOfWeek(today, { weekStartsOn: 1 });
        const twoWeeksAgoStart = new Date(currentWeekStart);
        twoWeeksAgoStart.setDate(twoWeeksAgoStart.getDate() - 14); // 2 Wochen zurück
        
        const nextWeekStart = new Date(currentWeekStart);
        nextWeekStart.setDate(nextWeekStart.getDate() + 7); // 1 Woche voraus
        
        // Erstelle Array mit allen 4 Wochen
        ...
        filtered = weeks.map(weekStart => {
          ...
          // Finde alle Daten in dieser Woche
        ...
        validData
          ...
          .forEach(d => {
            ...
            if (!monthData[monthKey]) {
              // Verwende den ersten Tag des Monats als Timestamp
      ...
      case "all":
        // Alle verfügbaren Daten, gruppiert nach Monaten
  ...
  // Berechne Gesamtnutzungszeit für den ausgewählten Zeitraum
  const totalMinutes = filteredChartData.reduce((sum, d) => sum + d.minutes, 0);
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;

  // Chart-Titel basierend auf Zeitraum
  const getChartTitle = () => {
    switch (chartPeriod) {
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage your workflow triggers and schedules</p>
        </div>
        <Button 
          size="sm" 
