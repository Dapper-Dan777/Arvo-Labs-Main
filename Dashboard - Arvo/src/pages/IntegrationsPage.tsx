import { Check, AlertTriangle, X, Plus, Settings, ExternalLink, Zap } from "lucide-react";
...
export default function IntegrationsPage() {
  ...
  const handleConnect = async (id: string) => {
    ...
    // Spezielle Behandlung für Gmail OAuth
    if (integration?.name.toLowerCase() === "gmail") {
      console.log('🟢 IntegrationsPage: Gmail OAuth wird gestartet');
      try {
        console.log('🟢 IntegrationsPage: Importiere startGmailOAuth...');
        const { startGmailOAuth } = await import("@/lib/gmail/oauth");
        console.log('🟢 IntegrationsPage: startGmailOAuth wird aufgerufen...');
        await startGmailOAuth();
        // startGmailOAuth redirects, so we won't reach here
        return;
      } catch (error) {
        console.error('❌ IntegrationsPage: Error starting OAuth:', error);
        toast({
          title: "Fehler",
          description: error instanceof Error ? error.message : "Fehler beim Starten der Gmail-Verbindung",
          variant: "destructive",
        });
        return;
      }
    ...
    // Standard Connect für andere Integrationen
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold">Integrations</h1>
      ...
      {filteredIntegrations.some(i => i.name.toLowerCase() === "gmail") && (
        <div className="mb-6">
              ...
              action={
                !searchQuery
                  ? {
                      ...
                      icon: Plus,
                      onClick: () => {/* TODO: Open add integration dialog */},
                    }
                  : undefined
              }
