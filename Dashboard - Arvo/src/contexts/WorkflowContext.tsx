...
export function WorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const executeWorkflow = async (id: number, getIntegrationService?: (name: string) => any, getIntegration?: (name: string) => any) => {
    ...
    if (!getIntegrationService) {
      const integrationModule = await import("@/services/integrations");
      getIntegrationService = integrationModule.getIntegrationService;
    }
    ...
    while (currentNodeId && stepCount < 100) {
      ...
      try {
        // Hole Integration Service - prüfe verschiedene mögliche Felder
        const nodeConfig = currentNode.data?.config || {};
        const serviceName = nodeConfig.service || nodeConfig.type || 
                           (currentNode.type === "trigger" ? "gmail" : "slack");
        
        const service = getIntegrationService(serviceName);
        
        if (service) {
          // Hole Integration für Credentials
          const integration = getIntegration ? getIntegration(serviceName) : { 
            status: "connected" as const,
            credentials: {},
          };
          
          // Erstelle WorkflowNode-Format für Service
          const workflowNode: WorkflowNode = {
            id: currentNode.id,
            type: currentNode.type as any,
          ...
          if (currentNode.type === "trigger") {
            triggerData = await service.executeTrigger(workflowNode, integration);
          } else if (currentNode.type === "action" || currentNode.type === "condition") {
            await service.executeAction(workflowNode, integration, triggerData);
          }
        } else {
          // Fallback: Simuliere Ausführung wenn kein Service gefunden
